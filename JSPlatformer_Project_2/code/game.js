...
function Level(plan) {
  ...
  this.player = this.actors.filter(function(actor) {
    return actor.type == "player";
      //return actor.type == new component ("Running_Bunny_Character.png", 10,120 "image");
  })[0];
}
...
var wobbSpeed = 3, wobbDist = 0.4;
    Health.prototype.act = function(step){
        ...
        this.pos =this.basePos.plus(new Vector(wobbPos, wobbPos))
    }
...
function runGame(plans, Display) {
  function startLevel(n) {
    ...
    runLevel(new Level(plans[n]), Display, function(status) {
      if (status == "lost")
         {document.write("You have died by lava!!! ");
          document.write("Refresh if you want to play again");
         }
      else if (n < plans.length - 1)
        startLevel(n + 1);
      else
      {document.write("You have beaten the game!!! ");
       document.write("Refresh if you want to play again");}
    });
  }
  startLevel(0);
